...
void draw(){
  ...
  if ( beat.isOnset() ) {
    ...
    if (random(1) > 0.6){
        println("Rotated to:", targetrot);
        targetscale  = random(0.8, 1.4);
    }
  }

  rot = lerp(rot, targetrot, 0.001);
  rotz = lerp(rotz, targetrotz, 0.001);
  scale = lerp(scale, targetscale, 0.01);

  int i=0;
  for(PVector p : points){
    stroke(255 * (i * 0.001), 0, 255 * (i * 0.02));
    strokeWeight(0.5);



    color co = img.get((int)p.x + width/2, (int)p.y + height/2);
    ...
    float offsetz = 0;
      ...
      offsety = map(noise(band + (i * 0.2)), 0, 1, band * -10, band * 10);
    //}
  ...
  position = PVector.lerp(position, target, 0.2);
